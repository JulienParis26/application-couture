...
class SellerResourceIT {
    ...
    public static Seller createEntity(EntityManager em) {
        Seller seller = new Seller().name(DEFAULT_NAME).webSite(DEFAULT_WEB_SITE);
        return seller;
    }
    ...
    public static Seller createUpdatedEntity(EntityManager em) {
        Seller seller = new Seller().name(UPDATED_NAME).webSite(UPDATED_WEB_SITE);
        return seller;
    }
    ...
    void createSeller() throws Exception {
        ...
        assertThat(testSeller.getWebSite()).isEqualTo(DEFAULT_WEB_SITE);
    }
    ...
    void getAllSellersWithEagerRelationshipsIsEnabled() throws Exception {
        when(sellerServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restSellerMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(sellerServiceMock, times(1)).findAllWithEagerRelationships(any());
    }
    ...
    void putNewSeller() throws Exception {
        ...
        updatedSeller.name(UPDATED_NAME).webSite(UPDATED_WEB_SITE);
        ...
        assertThat(testSeller.getWebSite()).isEqualTo(UPDATED_WEB_SITE);
    }
    ...
    void fullUpdateSellerWithPatch() throws Exception {
        ...
        partialUpdatedSeller.name(UPDATED_NAME).webSite(UPDATED_WEB_SITE);

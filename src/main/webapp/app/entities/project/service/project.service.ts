...
export class ProjectService {
  ...
  protected convertDateFromClient(project: IProject): IProject {
    return Object.assign({}, project, {
      creationDate: project.creationDate?.isValid() ? project.creationDate.format(DATE_FORMAT) : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((project: IProject) => {
        project.creationDate = project.creationDate ? dayjs(project.creationDate) : undefined;
      });
    }
    return res;
  }
